...
export class UserRepository {
    ...
    async createNewUser(userDto: UserDto, hashPassword: string): Promise<UserWithLogin> {
        ...
        try {
            ...
            const savedCredentials = await queryRunner.manager.save(CredentialsEntity, {
                login: userDto.login,
    ...
    async updateUser(
        ...
        try {
            if (userId) {
                await queryRunner.manager.update(UserEntity, {id: userId}, {...newInfo})
            }
            if (credentialsId) {
                await queryRunner.manager.update(CredentialsEntity, {id: credentialsId}, {...credentials})
            }
            await queryRunner.commitTransaction();
        } catch(err) {
    ...
    async getUsers(page: number, pageSize: number): Promise<UserEntity[]> {
        try {
            ...
            throw new InternalServerErrorException("Не удалось получить пользователей");
        }
    }
}